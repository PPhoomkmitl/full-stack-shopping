...
<body>
    ...
    <div class="container">
            <?php
                if ($_SESSION['admin'] !== 'user_admin') {
                    echo '<div>
                            ...
                            <form id="deleteForm" class="delete-form" action="category_delete_confirm.php" method="post">
                                <input type="hidden" name="list_id_category" id="selectedValues" value="">
                                <input type="hidden" name="total_id_category" id="selectedTotal" value="">
        ...
        <div>
            ...
            <?php
                if ($_SESSION['admin'] == 'user_admin') {
                    echo '<form class="add-category-form" action="category_insert_form.php" method="post">
                            <input type="submit" id="addCategoryButton" value="Add Category"/>
                        </form>';
                }
            ?>
    ...
    $cur = "SELECT product_type.id, product_type.name AS category_name, COUNT(product.ProID) AS product_count FROM product_type LEFT JOIN product ON product.product_type_id = product_type.id GROUP BY product_type.id, product_type.name;";
    ...
    if (mysqli_num_rows($msresults) > 0) {
        while ($row = mysqli_fetch_array($msresults)) {
            ...
            echo "<tr class='user-row'>
                    ...
                    <td>
                        <form class='action-button' action='category_update.php' method='post' style='display: inline-block;'>  
